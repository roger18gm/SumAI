...
def create_thread():
    ...
    try:
        ...
        if existing_thread_id:
            ...
            result = loop.run_until_complete(assistant.update_website_context_async(existing_thread_id, website_url))
        else:
            logging.info(f"Creating new thread for URL {website_url}")
            result = loop.run_until_complete(assistant.create_thread(website_url))
...
def get_active_thread():
    ...
    try:
        active_thread_id = assistant.get_active_threads()
        return jsonify({"thread_ids": active_thread_id})
    except Exception as e:
